...
test_that("Calculate examples from the literature", {
  ...
  result <- cocron.n.coefficients(alpha=c(.857,.875,.800,.833), items=c(50,40,35,25), n=100, indep=FALSE, r=r)
...
test_that("Compare with results from AlphaTest", {
  ...
  #1
  result <- cocron.n.coefficients(alpha=c(.6,.7), items=c(50,100), n=c(150,200), indep=TRUE)
  ...
  result <- cocron.n.coefficients(alpha=c(.6,.5), items=c(35,30), n=90, indep=FALSE, r=r)
  expect_that(result@df, is_identical_to(1))
  expect_that(round(result@statistic,4), is_identical_to(2.8699))
  expect_that(round(result@p.value,4), is_identical_to(.0903))

  #3
  result <- cocron.n.coefficients(alpha=c(.9,.7), items=c(25,30), n=c(200,150), indep=TRUE)
  expect_that(result@df, is_identical_to(1))
  ...
  result <- cocron.n.coefficients(alpha=c(.5,.4), items=c(20,20), n=c(200, 100), indep=FALSE, r=r)
  expect_that(result@df, is_identical_to(1))
  expect_that(round(result@statistic,4), is_identical_to(1.4895))
  expect_that(round(result@p.value,4), is_identical_to(.2223))

  #5
  result <- cocron.n.coefficients(alpha=c(.9,.7,.8), items=c(15,15,12), n=c(100,150,130), indep=TRUE)
  expect_that(result@df, is_identical_to(2))
  expect_that(round(result@statistic,4), is_identical_to(31.3795))
  expect_that(round(result@p.value,4), is_identical_to(.0000))

  #6
  result <- cocron.n.coefficients(alpha=c(.7,.75,.7), items=c(10,10,10), n=c(100,120,130), indep=TRUE)
  expect_that(result@df, is_identical_to(2))
  ...
  result <- cocron.n.coefficients(alpha=c(.75,.7,.72), items=c(12,12,12), n=c(60,70,80), indep=FALSE, r=r)
  expect_that(result@df, is_identical_to(2))
  ...
  result <- cocron.n.coefficients(alpha=c(.7,.75,.72), items=c(13,14,15), n=c(140,130,110), indep=FALSE, r=r)
  expect_that(result@df, is_identical_to(2))
  ...
  #9
  result <- cocron.n.coefficients(alpha=c(.7,.8,.75,.77), items=c(15,15,20,20), n=c(100,100,200,200), indep=TRUE)
  expect_that(result@df, is_identical_to(3))
  ...
  result <- cocron.n.coefficients(alpha=c(.75,.72,.70,.77), items=c(15,15,15,15), n=c(100,100,150,150), indep=FALSE, r=r)
  expect_that(result@df, is_identical_to(3))
...
test_that("Calculate examples from the literature", {
  ...
  result <- cocron.two.coefficients(alpha=c(.82,.89), n=27, indep=FALSE, r=.74, alternative="two.sided")
